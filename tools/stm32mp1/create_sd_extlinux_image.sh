...
sudo dd if=tf-a-${sBoard}.stm32 of=$sBoard.img seek=34 conv=notrunc
(($? != 0)) && { printf '%s\n' "Can't write TF-A FSBL1 Image"; exit 1; }
sudo dd if=tf-a-${sBoard}.stm32 of=$sBoard.img seek=546 conv=notrunc
